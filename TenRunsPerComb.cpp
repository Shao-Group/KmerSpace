...
int main()
{
    ...
    for (int k = 5; k < 9; ++k)
    {
        ...
        vector<char *> kmerSpace_cpy(kmerSpace);
        for (int d = 1; d < k; ++d)
        {
            cout << "10 runs for k = " << k << " and d = " << d << ":\n";
            for (int j = 0; j < 10; ++j)
            {
                ...
                cout << "\nThe graph has an independent set of size " << num_of_indep_nodes << endl << endl;
                kmerSpace = kmerSpace_cpy;
            }
        }
        for (auto & i : kmerSpace_cpy)
        {
            delete [] i;
        }
    }
